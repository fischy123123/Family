...
function buildModeInstruction(input: PlanDayInput): string {
  ...
  if (input.mode === 'suggest') {
    const capacity = isToday
      ? `the time still available TODAY (from the current time onward — respect what's already passed)`
      : `${dayLabel}, a FUTURE day whose WHOLE day is open — judge capacity by that day's plan and anchors, NOT by the current time or today's schedule`
    return `\n\nTASK — SUGGEST OPTIONAL ADDITIONS (do NOT modify the plan, do NOT output items): Here is ${dayLabel}'s plan so far:\n${current}\nLooking hard at the user's GOALS, STANDING COMMITMENTS, recent reflections, and overall life balance, propose up to 4 OPTIONAL things they could consider fitting into ${capacity}, that are NOT already in the plan. Strongly favor things that advance a stated goal or honor a standing commitment, or that restore an obviously neglected balance (movement, connection, rest, a put-off priority). Each is short and concrete with a one-line "why" that names the goal/commitment/benefit it serves. Be realistic about that day's capacity — if it's genuinely full or nothing meaningful fits, return fewer or an empty list. Never invent busywork or repeat anything already planned.\nOutput ONLY this JSON and nothing else: {"suggestions":[{"title":"...","why":"..."}]}`
  }
