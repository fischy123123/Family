  ...
  "items": [
    {
      ...
      "detail": "optional — 1-2 sentences of expanded context (timing, prep, who to contact, what to bring). Include only when there is genuinely useful elaboration beyond the title and reason. Omit when the title and reason already tell the full story.",
      ...
      "section": "REQUIRED on every item. The first name of the family member this item primarily concerns — must match exactly a name from FAMILY MEMBERS — or 'Family' for shared logistics, multi-person items, or things that genuinely don't belong to one person. Determines which person's section this card appears under in the briefing."
    }
  ],
  "problems": [
    {
...
- SECTION ASSIGNMENT (required): Every item must have a "section" field. Set it to the first name of the family member the item most directly concerns (typically whoever appears in forEmails, or the person directly affected). Use "Family" only when the item genuinely affects multiple members equally or doesn't belong to one person. Grouped cards (same groupKey) that span multiple people should use "Family" as their section. In the Family section, when 3 or more items fall on the same naturally busy day, give them a shared groupKey (e.g. 'thu-jun-19') and groupTitle (e.g. 'Thursday — busy day') so they cluster into one collapsible day card.
