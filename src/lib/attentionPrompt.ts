  ...
  "items": [
    {
      ...
      "section": "REQUIRED. The first name of the child or person this item is ABOUT — not who is responsible for handling it. A parent driving a child to music lessons → section is the CHILD's name. A concern to raise at a child's therapy appointment → section is the CHILD's name. Use 'Family' only when the item genuinely involves multiple children equally, or is a household-wide task with no single primary subject. Must match exactly a name from FAMILY MEMBERS."
    }
  ],
  "problems": [
    {
...
- SECTION & KIND (required on all items): "section" = who the item is ABOUT, not who handles it. A child's lesson, appointment, or prep task goes under that child even if a parent executes it. A concern to raise at their therapy → under the child. Use "Family" only for multi-child activities or household chores with no single subject. "kind" = "action" for things the user must actively do (raise a concern, decide something, complete a step); "awareness" for things happening they should know about (event, pickup, logistics). In the Family section, when 3+ items fall on the same busy day, use a shared groupKey/groupTitle to cluster them.
