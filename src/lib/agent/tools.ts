...
export const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description: "Add or update a structured fact directly on a family member's profile — their medical info, routines, preferences, or personal notes. Use this instead of remember() for personal facts ABOUT a specific person. The data appears on their profile card in the app and feeds into their personal AI context.\n\nCall get_member_profile first to see what already exists so you don't add duplicates.\n\nFIELD DECISION RULE — pick the right bucket:\n- 'importantInfo': WHAT IS TRUE about this person — static reference facts they carry with them. Use for: diagnoses, medications (name + dose), medical devices, allergies, doctors, school name/grade, employer, emergency contacts, legal facts. Key test: would this still be true if you removed all time from the world?\n- 'routine': WHAT HAPPENS ON A SCHEDULE — a repeating time-pattern for this person. Use for: recurring classes, pickups/drop-offs, therapy appointments, training sessions, medication ADMINISTRATION schedules (e.g. 'Adderall XR — every school morning before breakfast'), regular work shifts. Key test: does this have a 'when' that repeats?\n- OVERLAP RULE: some things are both a fact AND a schedule (e.g. a medication). Store the identity in importantInfo (label: 'Adderall XR 20mg', value: '1 capsule') and the administration pattern in routine (title: 'Adderall XR', schedule: 'School mornings before breakfast'). Never cram both into one entry.\n- 'preference': something they like or dislike — food, activities, environments, social style\n- 'note': a freeform observation that doesn't cleanly fit any other field",
    input_schema: {
      type: 'object' as const,
      properties: {
